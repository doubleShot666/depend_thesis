 ...
 * Integration of MeterPU for energy and time measurements by : Sami Djouadi | University of St Andrews.
 */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
void ReadInputFile(char *inputFile) {
    ...
    std::ofstream
    fout("stokeprice.txt", std::ios::app);

    for (loopnum = 0; loopnum < numOptions; ++loopnum) {
        rv = fscanf(file, "%f %f %f %f %f %f %c %f %f", &optionData[loopnum].s, &optionData[loopnum].strike,
                    &optionData[loopnum].r, &optionData[loopnum].divq, &optionData[loopnum].v, &optionData[loopnum].t,
                    &optionData[loopnum].OptionType, &optionData[loopnum].divs, &optionData[loopnum].DGrefval);
...
main(int argc, char **argv) {
    ...
    Meter<CPU_Time> cpu_time_meter;
    Meter<PCM_Energy> cpu_energy_meter;
    Meter<NVML_Energy<>> gpu_energy_meter;

    cpu_time_meter.start();
    cpu_energy_meter.start();
    gpu_energy_meter.start();

    calculateOptionPrice(subData, chunkOptionPrices, chunkSize, NUM_ITERATIONS, num_threads);

    cpu_time_meter.stop();
    cpu_energy_meter.stop();
    gpu_energy_meter.stop();
    cpu_time_meter.calc();
    cpu_energy_meter.calc();
    gpu_energy_meter.calc();
    ...
    int count = 3;
    sendbuff = (float *) malloc(count * sizeof(float));
    recvbuff = (float *) malloc(count * sizeof(float));
    sendbuff[0] = cpu_time_meter.get_value()/1000000;
    sendbuff[1] = cpu_energy_meter.get_value()/1000;
    sendbuff[2] = gpu_energy_meter.get_value()/1000;

    MPI_Reduce(sendbuff, recvbuff, count, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        printf("cpu time %lf: , cpu energy: %lf, gpu energy: %lf\n",
               recvbuff[0] / size,
               recvbuff[1] / size,
               recvbuff[2] / size);
