...
void nornirloop(){
    int i, j;
    nornir::ParallelFor pf(current_nThreads, new nornir::Parameters(getParametersPath()));
    for (j=0; j<NUM_RUNS; j++) {
...
int bs_thread(void *tid_ptr) {
    ...
    fptype priceDelta;
    int tid = *(int *)tid_ptr;
    int start = tid * (numOptions / current_nThreads);
    int end = start + (numOptions / current_nThreads);
...
#define LINESIZE 64
    ...
    // Execute the area of interest multiple times with different number of threads
    for (int i = 1; i <= nThreads; ++i) {
        current_nThreads = i;
        printf("%d,", current_nThreads);
#ifdef ENABLE_PARSEC_HOOKS
        __parsec_roi_begin();
#endif

#ifdef ENABLE_THREADS
        MAIN_INITENV(,8000000,current_nThreads);
#ifdef WIN32
        HANDLE *threads;
        int *nums;
        threads = (HANDLE *) malloc (current_nThreads * sizeof(HANDLE));
        nums = (int *) malloc (current_nThreads * sizeof(int));

        for(i=0; i<current_nThreads; i++) {
            nums[i] = i;
            threads[i] = CreateThread(0, 0, bs_thread, &nums[i], 0, 0);
        }
        WaitForMultipleObjects(current_nThreads, threads, TRUE, INFINITE);
        free(threads);
        free(nums);
#else
        int *tids;
        tids = (int *) malloc (current_nThreads * sizeof(int));

        for(i=0; i<current_nThreads; i++) {
            tids[i]=i;
            CREATE_WITH_ARG(bs_thread, &tids[i]);
        }
        WAIT_FOR_END(current_nThreads);
        free(tids);
#endif //WIN32
#else //ENABLE_THREADS
#ifdef ENABLE_OPENMP
        {
            int tid=0;
            omp_set_num_threads(current_nThreads);
            bs_thread(&tid);
        }
#else //ENABLE_OPENMP
#ifdef ENABLE_TBB
        tbb::task_scheduler_init init(current_nThreads);
...
#ifdef ENABLE_FF
        map m;
        m.numberThreads = current_nThreads;
        m.run();
        m.wait();
#else //ENABLE_FF
