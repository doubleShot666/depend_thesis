...
#define LINESIZE 64
    ...
    skepu::Vector<fptype> sptprice_sk(sptprice, numOptions, false),
    ...
    skepu::Vector<fptype> otype_sk(otype, numOptions, false);
    skepu::Vector<OptionData> data_sk(data, numOptions, false);
    skepu::Vector<fptype> prices_sk(prices, numOptions, false);
    auto map = skepu::Map<7>(mapFunction);
    auto spec = skepu::BackendSpec{skepu::Backend::Type::Hybrid};

    printf("Ratio,Time,JoulesCpuAll,JoulesCoresAll,JoulesDramAll,JoulesGraphicAll,PCM_energy,NVML_Energy\n");
    for (float ratio = 0.95; ratio >= 0.0; ratio-=0.05) {
        printf("%f,", ratio);
        spec.setCPUThreads(55);
        spec.setCPUPartitionRatio(ratio);
        map.setBackend(spec);
